...
from ai_services.shared.dependencies.dependencies import get_admin_user
...
async def lifespan(app: FastAPI):
    ...
    from ai_services.shared.dependencies.dependencies import get_embedding_service, get_generation_service
    from ai_services.shared.config.config import config
    ...
    try:
        ...
        logger.warning(f"⚠️ Embedding service warmup failed: {e}")
    
    try:
        if config.use_real_generation:
            generation_service = get_generation_service()
            if hasattr(generation_service, 'warmup'):
                await generation_service.warmup()
    except Exception as e:
        logger.warning(f"⚠️ Generation service warmup failed: {e}")
...
async def _safe_warmup_services() -> bool:
    ...
    try:
        ...
        try:
            from ai_services.shared.dependencies.dependencies import comprehensive_warmup
        except ImportError:
        ...
        try:
            ...
            try:
                from ai_services.shared.dependencies.dependencies import enhanced_telemetry
...
async def _safe_auto_seed() -> bool:
    ...
    try:
        ...
        try:
            from ai_services.ingestion_pipeline.seed_data import main_advanced_seeding
            ...
            try:
                from ai_services.ingestion_pipeline import seed_knowledge_base
...
async def _safe_cleanup_services() -> None:
    ...
    try:
        from ai_services.shared.dependencies.dependencies import cleanup_enhanced_services
...
async def health() -> HealthResponse:
    ...
    try:
        ...
        try:
            from ai_services.shared.dependencies.dependencies import get_comprehensive_service_status
...
async def detailed_health() -> ServiceStatusResponse:
    ...
    try:
        ...
        memory_info = {}
        try:
            from ai_services.shared.dependencies.dependencies import monitor_ai_service_memory
...
async def ai_services_health():
    ...
    try:
        ...
        try:
            from ai_services.shared.dependencies.dependencies import get_generation_service
...
async def database_health():
    ...
    try:
        ...
        try:
            from data_layer.models.scylla.scylla_models import EnhancedConversationHistory
...
async def test_document_processing():
    ...
    try:
        from ai_services.ingestion_pipeline.document_processor import (
...
# Load only core AI service routers (chat/search now handled by Go microservices)
routers_to_load = [
    # Core AI services for internal use by Go microservices
    # ("app.api.endpoints.search", "router"),  # Moved to Go
    # ("app.api.endpoints.chat", "router"),    # Moved to Go  
]
...
# -----------------------------
# Internal API Endpoints for Go Services
# -----------------------------
...
class InternalChatRequest(BaseModel):
    ...
    route: str = "auto"
    top_k: int = 5
    # User context for therapeutic personalization
    user_name: Optional[str] = None
    user_role: Optional[str] = None
    care_context: Optional[Dict[str, Any]] = None

@app.post("/internal/search", tags=["internal"])
async def internal_search(request: InternalSearchRequest):
    """Internal search endpoint for Go microservices (no auth required)"""
    try:
        from ai_services.shared.dependencies.dependencies import get_knowledge_service
        ...
        return {
            ...
            "route_used": results.get("route", request.route),
...
@app.post("/internal/chat", tags=["internal"])
async def internal_chat(request: InternalChatRequest):
    """Internal chat endpoint for Go microservices (no auth required)"""
    try:
        from ai_services.shared.dependencies.dependencies import get_chatbot_service
        ...
        response = await chatbot_service.answer_user_message(
            user_id=request.user_id or "internal",  # Use provided user_id
            ...
            } if any([request.user_name, request.user_role, request.care_context, request.user_id]) else None
        )
        
        processing_time_ms = (time.time() - start_time) * 1000
        
        # Handle different response types (dict vs ChatResponse object)
        if hasattr(response, 'message'):
            # ChatResponse object
            ai_response = response.message
            confidence = getattr(response, 'confidence', 0.0)
            rag_used = getattr(response, 'has_context', False)
            sources = getattr(response, 'context_sources', [])
        else:
            # Dict response
            ai_response = response.get("answer", response.get("response", response.get("message", "")))
            confidence = response.get("confidence", 0.0)
            # Check for RAG usage from retrieval data
            retrieval_data = response.get("retrieval", {})
            sources = retrieval_data.get("results", [])
            rag_used = len(sources) > 0 and response.get("generation_used", False)
        ...
        logger.error(f"Internal chat failed: {e}")
        raise HTTPException(status_code=500, detail="Internal chat failed")
...
async def root():
    ...
    return {
        "message": "Core AI Services API",
        "version": "2.0.0", 
        "description": "Core AI microservice with sentence-transformers/all-mpnet-base-v2, qwen3-1.7b, and RAG pipeline - used internally by Go microservices",
        "uptime_seconds": time.time() - startup_time,
        "startup_info": {
            "services_initialized": app_state["services_initialized"],
            "initialization_time": app_state["initialization_time"],
        ...
        "endpoints": {
            ...
            "detailed_health": "/health/detailed - Comprehensive service status",
            "ai_services": "/health/ai-services - AI service specific health",
            "memory": "/health/memory - Memory usage monitoring", 
            "database": "/health/database - Database connection status",
            "docs": "/docs - API documentation",
            "admin": "/admin/* - Admin endpoints for seeding and maintenance",
            "dev": "/dev/* - Development testing endpoints",
        },
        "architecture": {
            "role": "Core AI microservice for RAG pipeline and ML operations",
            "consumers": "Go microservices (Chat API, Search API) via HTTP",
            "databases": "PostgreSQL, MongoDB Atlas, Redis for AI workloads",
        },
    }
