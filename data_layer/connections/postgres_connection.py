...
class PostgreSQLConnectionManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            use_pooling = database_config.postgresql.host not in ["localhost", "127.0.0.1"]
            ...
            self._engine = create_async_engine(database_config.postgresql.url, **engine_kwargs)
            ...
            logger.info(
                f"✅ PostgreSQL connected {pool_info}: {database_config.postgresql.host}:{database_config.postgresql.port}"
            )
...
async def get_postgres_session() -> AsyncGenerator[AsyncSession, None]:
    """Get PostgreSQL session with proper initialization."""
    ...
    if not manager._initialized:
        try:
            ...
            # For testing/demo purposes, we'll skip PostgreSQL dependency
